    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-gray-600">Visão geral dos seus estudos</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="card">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600">Tempo estudado (7 dias)</p>
        ...
        <div className="card">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600">Sequência (Streak)</p>
              <p className="text-2xl font-bold text-gray-900 mt-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {activeCycle && (
          <div className="card">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-bold text-gray-900">Ciclo Ativo</h2>
              <Link to="/study-cycle" className="text-sm text-primary-600 hover:text-primary-700">
                Ver detalhes →
              </Link>
            </div>
            <p className="text-gray-600 mb-4">{activeCycle.name}</p>
            {activeCycle.cycleItems && activeCycle.cycleItems.length > 0 && (
              <div>
                <p className="text-sm text-gray-500 mb-2">Próxima matéria:</p>
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-bold text-gray-900">Revisões Pendentes</h2>
            <Link to="/revisions" className="text-sm text-primary-600 hover:text-primary-700">
              Ver todas →
            </Link>
          </div>
            ...
            <div className="space-y-3">
              {pendingRevisions.map((revision) => (
                <div key={revision.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div>
                    <p className="font-medium text-gray-900">{revision.subject?.name}</p>
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link to="/statistics" className="text-sm text-primary-600 hover:text-primary-700">
            Ver todas →
          </Link>
        </div>
      ...
      {showSessionForm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <h2 className="text-2xl font-bold mb-4">Registrar Sessão de Estudo</h2>
            <StudySessionForm
